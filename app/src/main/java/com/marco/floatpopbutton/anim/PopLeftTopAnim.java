 ...
 * Time: 18:48
 */
public class PopLeftTopAnim implements IPosition {
    int measureHeight, normalHeight, padding, duration, alpha;
    private ArrayList<View> views  = new ArrayList<>();
    private int             radius = 40;

    public PopLeftTopAnim(int measureHeight, int normalHeight, int padding, ArrayList<View> views) {
        this(measureHeight, normalHeight, padding, 400, 1, views);
    }

    public PopLeftTopAnim(int measureHeight, int normalHeight, int padding, int duration, int alpha, ArrayList<View> views) {
        ...
        this.alpha = alpha;
        this.views = views;
        radius +=measureHeight;
    }

    @Override
    public void doAnimOut() {
        int start = -90;
        int degree = 90 / views.size();
